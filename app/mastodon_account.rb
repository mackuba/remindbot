...
class MastodonAccount
  ...
  OAUTH_SCOPES = 'read:accounts read:statuses write:statuses'
  ...
  def save_config
    Dir.mkdir(File.dirname(CONFIG_FILE)) unless File.exist?(File.dirname(CONFIG_FILE))
  ...
  def send_message(target_user, text)
    ...
    api.send_message(target_user, text)
  end
end
